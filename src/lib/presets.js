/* Presets ship as JSON under public/presets/ : presets/index.json lists the
   ...
   loadPresets returns { presets, groups, paths } — presets is the flat
   key→template map either way; groups mirrors the catalogue with resolved keys
   (entries that fail to load are dropped), or null for a flat index; paths is
   key→the file it came from, for the dev-only "save to source file".
  ...
  try{
    ...
    for(const f of files){
      try{
        ...
        const url=remote ? f : BASE+'presets/'+f
        ...
        // base: those come from another origin and their srcs were rewritten
        if(!remote&&!BASE) paths[key]=String(f)
    ...
    return {presets:out, groups:(groups&&groups.length)?groups:null, paths}
